...
importing the data with the `TPP` package [@Franken2016]. We showcase how this can be done in the `Rtpca` package
vignette.         
...
The crucial columns it contains are the following `protein`: a column using the same identifiers (Gene names (in this case), Uniprot ids or Ensembl protein ids) as the rownames of the supplied input matrix of ExpressionSet object and `id`: protein complex ids.            \\
...
```
We can plot a ROC curve to see how predictive our data is on recovering protein complexes by evoking:
...
Its essential columns are `x`: 1st interactor, `y`: 2nd interactor and `pair`: unique id of both interactors in alphabetical order (interactions should be unique).
...
In order to asses how many differential PPIs can be recovered with either of the approaches, we plot a Venn diagramm below: 
...
plot(venn(venn_df))
```
It appears that both approaches pick up a large set of distinct differential PPIs. The PPI-centric approach appears to recover more significant PPI changes, however the complex-centric one reveals more intra-complex centered interactions changes.
